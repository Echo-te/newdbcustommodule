...
class OigCustomer(models.Model):
    ...
    def create(self, vals):
        if vals.get('customer_id', 'New') == 'New':
            # Use the sequence defined for customer ID generation
    ...
    def process_commissions(self):
        """
        Process commissions for the customer. This method is called after each contribution creation.
        """
        # Define the commission period (e.g., monthly)
        today = date.today()
        start_of_month = today.replace(day=1)
        end_of_month = (start_of_month + timedelta(days=31)).replace(day=1) - timedelta(days=1)
        
        # Check if commission for the current month is already processed
    ...
    def process_commissions_button(self):
        """
        Button method to allow manual commission processings.
        """
